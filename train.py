...
parser.add_argument("--layer_dropout", type=float, default=0, help="GCN layer dropout rate.")
parser.add_argument(
...
parser.add_argument("--exp_id", type=str, default="res14", help="The experiment id")
...
def trainmodel(config=None):
    ...
    # args.save_dir = args.save_dir + '/bsz-{}-seed-{}-npseed-{}'.format(args.batch_size, args.seed, args.npseed)
    ...
    train_batch, test_batch = get_dataloaders(args, vocab)

    trainer = Trainer(args, emb_matrix=word_emb)
    print(_totally_parameters(trainer.model))
    if not tune_mode:
        print(trainer.model)

    best_path = args.save_dir + '/' + args.exp_id
    ...
    for _ in range(1, args.num_epoch + 1):
        ...
        val_loss, val_acc, val_f1 = evaluate(trainer, test_batch)
    ...
    # if not tune_mode:
    #     print(
    #         "Best best_acc_epoch:{}, test_loss:{}, test_acc:{}, test_f1:{}".format(
    #             bt_val_idx, bt_val_loss, bt_val_acc, bt_val_f1
    #         )
    #     )
    # else:
    #     tune.track.log(mean_accuracy=bt_val_acc, f1=bt_val_f1)

    # bt_val_f1 = max(val_f1_score_history)
    # bt_val_f1_idx = val_f1_score_history.index(bt_val_f1)
    # bt_val_acc = val_acc_history[bt_val_f1_idx]
    # bt_val_loss = val_loss_history[bt_val_f1_idx]
    # if not tune_mode:
    #     print(
    #         "Fold:{}, Best best_f1_epoch:{}, test_loss:{}, test_acc:{}, test_f1:{}".format(
    #             1, bt_val_f1_idx, bt_val_loss, bt_val_acc, bt_val_f1
    #         )
    #     )
    
    trainer = torch.load(best_path + '/best_checkpoint.pt')
    test_loss, test_acc, test_f1 = evaluate(trainer, test_batch)
    print("Final Result: test_loss:{}, test_acc:{}, test_f1:{}".format(test_loss, test_acc, test_f1))

    # draw_curve(train_log=train_acc_history, val_log=val_acc_history[1:], curve_type="acc", epoch=args.num_epoch)
    # draw_curve(train_log=train_loss_history, val_log=val_loss_history[1:], curve_type="loss", epoch=args.num_epoch)
...
if __name__ == "__main__":
    if tune_mode:
        print(args)
        hyperparameters = {
            "bsz": tune.grid_search([32]),
            "seed": tune.grid_search([2020]),
            "inp_drop": tune.grid_search([0.7]),
            "npseed": tune.grid_search(list(range(0, 51))),
        }
        analysis = tune.run(
            trainmodel, config=hyperparameters, resources_per_trial={"cpu": 3, "gpu": 0.33}
        )
